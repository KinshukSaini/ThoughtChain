...
// Helper to get model with optional custom API key
function getModel(customApiKey?: string) {
...
export async function generateResponse(input: string, messages: Array<{role: string, content: string}>, customApiKey?: string) {
...
export async function decideCreateNode(input: string, state: any, customApiKey?: string) {
  const model = getModel(customApiKey);
